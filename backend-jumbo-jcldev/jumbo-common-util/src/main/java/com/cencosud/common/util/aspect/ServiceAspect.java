package com.cencosud.common.util.aspect;
...
public class ServiceAspect {
	...
	@Around("execution(* *(..)) && @annotation(com.cencosud.common.util.aspect.Loggable)")
	public Object aroundCreation(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			valorRetorno = joinPoint.proceed();
		} catch (Throwable ex) {
			throw ex;
		} finally {
			LOGGER.info("Fin: [{}] ({})", joinPoint.getTarget().getClass().getName(),
					joinPoint.getSignature().getName());
		}
