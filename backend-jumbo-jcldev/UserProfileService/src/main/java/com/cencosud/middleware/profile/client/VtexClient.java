package com.cencosud.middleware.profile.client;
...
import com.cencosud.middleware.profile.model.Pair;
...
public class VtexClient {
	...
	public <T> T execute(String path, List<NameValuePair> nvps, RequestProtocolEnum protocol,
			TypeReference<T> reference) throws UserProfileException {
		try {
			...
			throw new UserProfileException("Error al intentar decodificar la informacion", e);
		}
	}
	...
	public Pair<String, Map<String, String>> executeAndGetResultAsStringAndHeaders(String path,
			List<NameValuePair> nvps, RequestProtocolEnum protocol) throws UserProfileException {
		try {
			HttpResponse response = this.executeRequest(path, nvps, protocol);
			StringBuilder content = new StringBuilder();
			BufferedReader in = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
	...
	private HttpResponse executeRequest(String path, List<NameValuePair> nvps, RequestProtocolEnum protocol)
		...
		builder.setScheme(schema).setHost(connectionUrl).setPath(path).addParameters(nvps);
