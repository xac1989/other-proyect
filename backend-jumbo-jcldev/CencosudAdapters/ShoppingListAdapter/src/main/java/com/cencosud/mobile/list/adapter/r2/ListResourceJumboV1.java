package com.cencosud.mobile.list.adapter.r2;
...
@Api(value = "List Adapter Resource Jumbo V1")
@Path("/r2/v1")
public class ListResourceJumboV1 {
	private final Logger logger = Logger.getLogger(ListResourceJumboV1.class.toString());
    ...
    @Produces({ MediaType.APPLICATION_JSON })
    @OAuthSecurity(enabled = true, scope = "customer")
    public Response putList(ListProductUpdateRequest request, @Context AdapterSecurityContext securityContext) throws NotFoundException {
        AuthenticatedUser user = securityContext.getAuthenticatedUser();
        Map<String, Object> attributes = user.getAttributes();
        String userId = getAttributeFromContext(attributes, "userId");
        if(userId == null){
