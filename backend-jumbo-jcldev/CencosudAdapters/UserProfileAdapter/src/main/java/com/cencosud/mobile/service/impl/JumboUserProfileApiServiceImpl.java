...
import com.cencosud.mobile.model.profile.jumbo.UserMigration;
import com.cencosud.mobile.model.profile.jumbo.UserMigrationResponse;
...
public class JumboUserProfileApiServiceImpl implements UserPreferenceApiService {

    private static final String PROFILE_PATH_PRE = "userprofile/";
    private static final String PROFILE_PATH_POS = "/v1/profile";
    ...
    private static final String SAVE_PREFERENCES_PATH = "/userprofile/r2/v1/profile/preferences";
    private static final String SALES_CHANNEL_LIST = "/saleschannel/r2/v1/salesChannel/";
    private static final String USER_MIGRATION = "/%s/v1/usermigration/%s";
    ...
    static final Logger LOGGER = LoggerFactory.getLogger(JumboUserProfileApiServiceImpl.class);
    ...
    public void savePreferences(UserPreferencePostRequest request) {
        restServiceUtil.executeService(SAVE_PREFERENCES_PATH, request, Void.class, null, RequestProtocolEnum.POST);
    }
    ...
    @Override
    public boolean getNeedsMigration(String rut, String region) {
        boolean result = false;
        String path = String.format(USER_MIGRATION, region, rut);
        try {
            UserMigrationResponse userMigrationResponse = restServiceUtil.executeService(path, null,
                    UserMigrationResponse.class, null, RequestProtocolEnum.GET);
            if (userMigrationResponse != null) {
                UserMigration userMigration = userMigrationResponse.getUserMigration();
                LOGGER.info("User migration code: {}", (userMigration != null ? userMigration.getCode() : 0));
                if (userMigration != null && userMigration.getCode() == 1) {
                    result = userMigration.isNeedsMigration();
                }
            }
        } catch (Exception e) {
            LOGGER.warn("Error encountered when requesting user migration service.", e);
        }
        return result;
    }
}
