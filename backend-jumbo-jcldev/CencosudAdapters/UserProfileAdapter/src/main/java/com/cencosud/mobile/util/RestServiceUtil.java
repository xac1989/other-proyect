...
public class RestServiceUtil {
	...
	static final Logger LOGGER = LoggerFactory.getLogger(RestServiceUtil.class);
	...
	public <T, E> E executeService(String path, T document, Class<E> responseClass, Map<String, Object> queryParams,
			RequestProtocolEnum method) {
		try {
			Client client = ClientBuilder.newClient();
			client.property(ClientProperties.CONNECT_TIMEOUT, 2500);
		    client.property(ClientProperties.READ_TIMEOUT,    2500);
			WebTarget fullTextTarget = client.target(apiBaseUrl).path(path);
			if (queryParams != null) {
			...
			LOGGER.info("URL: {}", fullTextTarget.getUri().toString());
			...
			LOGGER.info("Se ejecutó el servicio.............................."+response.getStatus());
			...
			throw new RuntimeException("Error " + Status.NOT_FOUND.getStatusCode() + e.getMessage());
		}
	}
