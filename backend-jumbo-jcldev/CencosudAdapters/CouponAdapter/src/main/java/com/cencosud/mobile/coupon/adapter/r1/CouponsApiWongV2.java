...
@Api(value = "Coupon API V2")
@Generated(value = "class com.github.mfpdev.adapters.swagger.codegen.MfpAdapterCodegen", date = "2017-01-09T16:45:59.781-03:00")
@Component
public class CouponsApiWongV2 {

	private final Logger logger = Logger.getLogger(CouponsApiWongV2.class.toString());
	...
	private String getDni(AdapterSecurityContext securityContext) throws NotFoundException {
		...
		dni = this.getUserProfileApiService().profileGet(email,REGION,"v2").getDocument();
	...
	private String getEmail(AuthenticatedUser authenticatedUser) throws NotFoundException {
		...
		if (constainsValidEmail(authenticatedUser)) {
			email = ((String) authenticatedUser.getAttributes().get("email")).trim();
	...
	private boolean constainsValidEmail(AuthenticatedUser authenticatedUser) {
		return authenticatedUser != null && authenticatedUser.getAttributes() != null
