...
public class JumboClLoginAdapter extends UserAuthenticationSecurityCheck {

	private static final Logger logger = LoggerFactory.getLogger(JumboClLoginAdapter.class);
	...
	protected boolean validateCredentials(Map<String, Object> credential) {
		...
		if (isValidCredential(credential)) {
			...
			try {
				if (!isEmail(userName)) {
					UserMigration userMigration = getConfiguration().getLoginService().getUserMigration(userName);

					if (userMigration.getNeedsMigration()) {

						attributes = new HashMap<String, Object>();
						attributes.put("email", userMigration.getEmail());
						return true;
					} else {
						if (userMigration.getEmail() != null) {
							userName = userMigration.getEmail();
						} else {
							return false;
						}
	...
	public SecurityCheckConfiguration createConfiguration(Properties properties) {
		return new JumboClLoginConfiguration(properties);
	}

	@Override
	protected JumboClLoginConfiguration getConfiguration() {
		return (JumboClLoginConfiguration) super.getConfiguration();
